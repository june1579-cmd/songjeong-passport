...
// 카카오톡 알림 발송 기록을 남긴다(실제 발송 연동 전까지는 notifications 테이블에 pending으로 기록만 됨 —
// 카카오 비즈니스 계정/알림톡 API 연동 후 이 함수의 마지막 insert 부분만 실제 발송 API 호출로 교체하면 된다).
export async function promoteNextWaitlisted(programId: string, freedRegistrationStatus: string) {
  ...
  const { data: participant } = await supabase.from("participants").select("name").eq("id", (nextInLine as Registration).participant_id).single();
