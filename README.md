...
## 14. 실사용 전환 (3) — 알리고(Aligo) SMS 실제 연동

휴대폰 인증(`/api/otp/request`)과 대기자 자동 알림(`src/lib/waitlist.ts`)이 이제 알리고 API와 바로 연동되도록 만들어져 있다.

**사업자 인증 승인 후 할 일은 딱 하나 — Vercel 환경변수 3개 추가뿐이다.** 코드는 이미 다 준비되어 있다.

```
ALIGO_API_KEY=알리고에서 발급받은 API Key
ALIGO_USER_ID=알리고 로그인 아이디
ALIGO_SENDER=사전 등록한 발신번호 (하이픈 없이 숫자만, 예: 0517496304)
```

3개를 모두 넣고 Redeploy하면:
- 회원가입 시 실제로 문자로 인증번호가 발송된다 (화면에 인증번호가 더는 표시되지 않는다)
- 대기자가 자동 선정될 때 실제로 문자 알림이 간다 (신청자 관리 화면의 "알림 발송 기록"에 "발송됨"으로 표시된다)
